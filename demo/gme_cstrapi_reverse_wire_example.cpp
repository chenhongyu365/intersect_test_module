/*********************************************************************
 * @file    gme_cstrapi_reverse_wire_example.cpp
 * @brief
 * @details
 * @author  杨楠
 * @date    2024.7.29
...
//
// APIs:
//    gme_api_reverse_wire
//
// Description:
//    随机生成线段，组成一个开放的wire，分别将反转前后的COEDGE的中点和终点连线画成红色和绿色。
// **********************************************************************************
outcome aei_REVERSE_WIRE_DEMO1(ENTITY_LIST& output_ents, AcisOptions* ptrAcisOpt /*= nullptr*/) {
...
// **********************************************************************************
outcome aei_REVERSE_WIRE_DEMO2(ENTITY_LIST& output_ents, AcisOptions* ptrAcisOpt /*= nullptr*/) {
    BODY* body = nullptr;                                        // 用于接收创建的BODY
    WIRE* wire = nullptr;                                        // 用于接收创建的WIRE
    std::default_random_engine generator;                        // 随机数生成器
    std::uniform_real_distribution<double> distribution(-5, 5);  // 随机数生成器
